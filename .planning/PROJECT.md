...
## Current Milestone: v5.0 Imperative Ergonomics

**Goal:** v4.0 가변 변수와 자연스럽게 어우러지는 명령형 구문 요소 추가 — 배열 인덱싱 구문, 표현식 시퀀싱, 루프, 조건부 단순화

**Target features:**
- `arr.[i]` / `arr.[i] <- v` — 배열 인덱싱 구문 (array_get/array_set 대체)
- `e1; e2` — 표현식 시퀀싱 연산자 (`let _ = e1 in e2`로 디슈가링)
- `while cond do body` / `for i = start to end do body` — 루프 구문
- `if cond then expr` — else 없는 조건문 (unit 반환)

## Context

**Shipped:** v4.0 Mutable Variables (2026-03-27)
- 23+ source files, ~12,000 lines of F#
- 224 F# unit tests + 551 flt tests, all passing, 0 warnings
- 44 phases, 103 plans executed across v1.0-v4.0
