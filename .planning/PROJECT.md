...
- [ ] Module-level 파라미터 타입 어노테이션 (`let f (x : int) (y : bool) = ...`)
- [ ] Module-level 반환 타입 어노테이션 (`let f x : int = ...`)
- [ ] 앵글 브래킷 제네릭 구문 (`Result<'a>`, `Map<string, int>`)
...
## Current Milestone: v8.0 Declaration Type Annotations

**Goal:** FunLexYacc 호환성을 위해 module-level 함수 선언에 타입 어노테이션과 앵글 브래킷 제네릭 구문 지원 추가

**Target features:**
- 파라미터 타입 어노테이션: `let f (x : int) (y : bool) = ...`
- 반환 타입 어노테이션: `let f x : int = ...`
- 앵글 브래킷 제네릭: `type Result<'a> = ...`, `Result<'a>` 타입 표현식
...
---
*Last updated: 2026-03-30 after v8.0 milestone started*
