...
### Active — v2.0 Practical Language Completion

**Goal:** FunLexYacc 프로젝트에서 발견된 34개 제약사항 전면 해결. LangThree를 실용적 프로젝트에서 해킹 없이 사용 가능한 언어로.

**P0 — Blocking:**
- Import mechanism (`open Module` / 파일 간 모듈 로딩)
- Multiple module declarations in concatenated files
- Empty .fun file crash fix
- Prelude available outside LangThree directory
- Module scoping for type name collision resolution
- File I/O (`read_file`, `stdin_read_all`, `stdin_read_line`)

**P1 — High Impact:**
- N-tuples (3+ elements) + let-tuple destructuring
- Char type + `char_to_int` / `int_to_char` + char comparison
- Multi-line list literals
- Local `let rec ... in` inside function bodies
- String comparison operators (`<`, `>`, `<=`, `>=`)
- `write_file`, `append_file`, `file_exists`

**P2 — Nice to Have:**
- `()` unit literal as function argument (`f ()`)
- `failwith` builtin
- List literal patterns (`[x]`, `[x; y]`)
- Trailing semicolons in lists
- `else` + expression keywords (`match`, `if`, `let`, `try`, `fun`)
- Deep nesting parser improvement
- Top-level `let ... in` after module code
- `option` as alias for `Option`
- `read_lines`, `write_lines`, `get_args`, `get_env`, `get_cwd`, `path_combine`, `dir_files`
...
- do binding
- Mutable data structures (array, hashtable)
- Seq expressions
...
**v2.0 동기**: FunLexYacc 프로젝트(렉서/파서 생성기)에서 LangThree를 실제 사용하며 발견된 24개 제약사항. cat/sed 해킹, 접두사 규칙, 26개 등호 체인 등 workaround 제거가 목표.
...
---
*Last updated: 2026-03-24 after v2.0 requirements expanded (34 items)*
