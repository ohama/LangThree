...
- Polymorphic GADT return types (`eval : 'a Expr -> 'a` — OCaml `type a.` 스타일)
...
**Shipped:** v1.7 F#-Style Offside Rule & List Syntax (2026-03-22)
- 23 source files, 10,624 lines of F#
- 635 tests (196 F# + 439 fslit), all passing
- 24 phases, 62 plans executed across v1.0-v1.7
- F#-style implicit `in` via offside rule, list separator `,` → `;`
...
**Goal:** OCaml 스타일 다형적 GADT 반환 — `eval : 'a Expr -> 'a`처럼 입력 타입에 따라 반환 타입이 달라지는 함수 지원

**Target features:**
- synth 모드에서 GADT match를 fresh type variable로 check 위임
- 타입 주석에 타입 변수 허용 (`(match ... : 'a)`)
- 분기별 독립적 타입 정제 (IntLit → `'a=int`, BoolLit → `'a=bool`)
- 테스트 및 튜토리얼 업데이트

---
*Last updated: 2026-03-23 after v1.8 milestone started*
