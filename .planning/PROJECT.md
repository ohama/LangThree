...
## Current Milestone: v2.2 Module Access Fix & Test Coverage

**Goal:** Qualified module access 버그 수정 (E0313), 누락 테스트 보충

**Target features:**
- 임포트된 파일 모듈의 qualified access 수정 (`open "file.fun"` 후 `Module.func` 호출)
- Prelude qualified access 수정 (`List.length`, `List.map` 등)
- failwith 인라인 try-with 파싱 수정
- 누락된 flt 테스트 보충 (failwith, LetPatDecl, qualified module access)

## Context

**Shipped:** v2.1 Bug Fixes & Test Hardening (2026-03-25)
- 23+ source files, 11,569 lines of F#
- 214 F# unit tests + 468 flt tests, all passing, 0 warnings
- 35 phases, 87 plans executed across v1.0-v2.1

**Known Issues (post v2.1):**
- E0313: 임포트된 파일 모듈 qualified access 실패 (`open "file.fun"` → `Module.func` → E0313)
- E0313: Prelude qualified access 실패 (`List.length`, `List.map`, `List.head` 모두 실패)
- failwith 인라인 try-with 파싱 에러 (`try failwith "boom" with e -> "caught"` → parse error)
- Phase 26(failwith), Phase 28(LetPatDecl) flt 테스트 없음
...
---
*Last updated: 2026-03-25 after v2.2 milestone started (module access fix & test coverage)*
