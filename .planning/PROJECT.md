...
- [ ] Native StringBuilder (네이티브 문자열 빌더)
- [ ] Native Dictionary<K,V> (해시맵, .TryGetValue, .Keys, .Count 등)
- [ ] Native HashSet<T> (.Add, .Contains, .Count)
- [ ] Native Queue<T> (.Enqueue, .Dequeue, .Count)
- [ ] Native List<T> mutable (가변 리스트, .Add, indexing, .Count)
- [ ] .Length 프로퍼티 (문자열/배열)
- [ ] String slicing s.[start..end]
- [ ] List comprehension [for x in coll -> expr]
- [ ] 네이티브 컬렉션 for-in 반복
- [ ] sprintf/printfn 포맷 문자열 강화
- [ ] String/Char 메서드 (.EndsWith, .Trim, IsDigit, ToUpper 등)
- [ ] Prelude 함수 확장 (List.sort, List.sortBy, List.mapi, List.distinctBy 등)
...
**Shipped:** v7.0 Native Collections & Built-in Library (2026-03-29)
...
---
*Last updated: 2026-03-29 after v7.0 milestone start*
