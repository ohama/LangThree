...
- `--check` 플래그: 타입 체크만 (실행 없음), 모든 임포트 파일 포함
- `--deps` 플래그: 의존성 트리 출력
- `--prelude` 플래그 + `LANGTHREE_PRELUDE` 환경 변수
- 파일 임포트 캐싱 (동일 프로세스 내 중복 로드 방지)
- `funproj.toml` 프로젝트 파일 파싱 (TOML)
...
## Current Milestone: v9.0 Project Build System

**Goal:** `funproj.toml` 기반 Cargo 스타일 빌드 시스템으로 멀티파일 프로젝트 체계적 관리

**Target features:**
- CLI 확장: `--check`, `--deps`, `--prelude` 플래그
- 파일 임포트 캐싱 (중복 로드 방지)
- `funproj.toml` 프로젝트 파일 (TOML)
- `langthree build/test` 서브커맨드
- 다중 타겟 ([[executable]], [[test]])
...
---
*Last updated: 2026-03-31 after v9.0 milestone started*
