...
- [ ] Dot notation 제거 — 모든 .Property/.Method() dispatch를 module function으로 대체
- [ ] 누락 builtin 추가 (string_endswith, string_startswith, string_trim, hashtable_trygetvalue, hashtable_count)
- [ ] StringBuilder.append 이름 충돌 해결 (List.append와 스코프 충돌)
- [ ] Hashtable for-in tuple 방식 전환 (KeyValuePair .Key/.Value → (k, v) 튜플)
- [ ] Prelude 모듈 확장 (String.fun, Hashtable.fun)
- [ ] Eval.fs/Bidir.fs value-type FieldAccess dispatch 코드 제거
- [ ] flt 테스트를 module function 방식으로 전환
...
## Current Milestone: v7.1 Remove Dot Notation

**Goal:** OOP 스타일 dot dispatch를 제거하고 순수 함수형 module function API로 통일

**Target features:**
- 누락 builtin 함수 추가 (string/hashtable)
- Prelude 모듈 확장 (String, Hashtable, StringBuilder)
- Hashtable for-in → tuple destructuring
- Value-type FieldAccess dispatch 코드 제거
- 기존 테스트 module function 방식 전환

**Previous:** v7.0 Native Collections & Built-in Library (2026-03-29)
- 90 files changed, +9,857 LOC in v7.0
- 633 flt tests
- 59 phases, 126 plans executed across v1.0-v7.0
...
| Dot notation 제거 | OOP 스타일 제거, 순수 함수형 API 통일 — FunLexYacc는 별도 마일스톤에서 수정 | — Pending |

---
*Last updated: 2026-03-29 after v7.1 milestone start*
