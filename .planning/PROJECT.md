...
## Current Milestone: v2.1 Bug Fixes & Test Hardening

**Goal:** v2.0에서 발견된 런타임 버그 수정 및 테스트 커버리지 보충으로 코드 품질 확보

**Target features:**
- TCO regression 수정 (LetRec/LetRecDecl BuiltinValue의 tailPos=false 버그)
- 단위 테스트 격리 수정 (typeCheckModule 전역 상태 간섭)
- Phase 29-32 누락 flt 테스트 보충 (char, parser, file import, file I/O, system builtins)
- F# 컴파일 경고 정리 (FS0025 incomplete pattern match 등)

## Context

**Shipped:** v2.0 Practical Language Completion (2026-03-25)
- 23+ source files, 11,574 lines of F#
- 214 F# unit tests, 447 flt tests
- 32 phases, 82 plans executed across v1.0-v2.0
- 34 v2.0 requirements shipped (34/34)

**Known Issues (v2.0 post-release):**
- TCO 완전히 깨짐: Phase 30에서 LetRec을 BuiltinValue로 변경하면서 tailPos=false 하드코딩 → 1M iteration stack overflow
- F# 단위 테스트 1개 비결정적 실패: deep nested constructor match (병렬 실행 시 상태 간섭)
- flt 테스트 2개 실패: tco-simple-loop.flt, tco-match.flt (TCO 버그)
- Phase 29(char), 30(parser), 31(file import), 32(file I/O, system builtins) flt 테스트 없음
- IntegrationTests.fs에 FS0025 경고 20개
...
---
*Last updated: 2026-03-25 after v2.1 milestone started (bug fixes & test hardening)*
