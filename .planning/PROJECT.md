...
- LetRecDecl AST 확장 — 첫 파라미터 타입 어노테이션 보존 (`(string * string * Expr * Span)` → 타입 정보 포함)
- Expression-level mutual recursion (`let rec f x = ... and g y = ... in expr`) — AST + Parser + Bidir + Eval 전체 구현
...
## Current Milestone: v8.1 Mutual Recursion Completion

**Goal:** v8.0에서 도입한 타입 어노테이션을 `let rec ... and ...`에 완전히 적용하고, expression-level 상호 재귀를 구현

**Target features:**
- LetRecDecl AST에 첫 파라미터 타입 정보 보존
- Expression-level `let rec f x = ... and g y = ... in expr` 상호 재귀
- 타입 어노테이션과 상호 재귀의 완전한 조합 지원
...
---
*Last updated: 2026-03-31 after v8.1 milestone started*
