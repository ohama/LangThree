...
- LetRecDecl/LetRec AST에 TypeExpr option 추가 — 첫 파라미터 타입 어노테이션 보존 — v8.1
- elaborateTypeExpr로 첫 파라미터 타입 어노테이션 실제 검증 (TypeCheck/Bidir/Infer) — v8.1
- Expression-level `let rec ... and ... in expr` 상호 재귀 (AST bindings list + LetRecContinuation 재사용) — v8.1
- Bidir/Infer simultaneous env + Eval sharedEnvRef 패턴 — v8.1
- `--check` 플래그: 타입 체크만 (실행 없음), 모든 임포트 파일 포함 — v9.0
- `--deps` 플래그: 의존성 트리 출력 — v9.0
- `--prelude` 플래그 + `LANGTHREE_PRELUDE` 환경 변수 — v9.0
- 파일 임포트 캐싱 (동일 프로세스 내 중복 로드 방지) — v9.0
- `funproj.toml` 프로젝트 파일 파싱 (Tomlyn 2.3.0 TOML) — v9.0
- `langthree build [name]` / `langthree test [name]` 서브커맨드 — v9.0
- `[project].prelude` funproj.toml Prelude 경로 설정 — v9.0
- AST Span 위치 정보 수정 (PositionedToken + IndentFilter 위치 보존 + lexbuf 업데이트) — v9.1

### Active

- 타입 클래스 (Haskell 스타일 ad-hoc polymorphism)
- 딕셔너리 전달 방식 인스턴스 해결
- 기존 to_string, 비교 연산자 등의 다형성 문제 해결
...
## Current Milestone: v10.0 Type Classes

**Goal:** Haskell 스타일 타입 클래스로 ad-hoc polymorphism 지원 — to_string, 비교, 수학 연산의 다형성 문제 근본 해결

**Target features:**
- 타입 클래스 선언 (`typeclass Show 'a = ...`)
- 인스턴스 선언 (`instance Show int = ...`)
- 딕셔너리 전달 방식 인스턴스 해결
- 제약 조건부 인스턴스 (`instance Show (Option 'a) where Show 'a`)
- 기존 to_string/비교 연산의 타입 클래스 통합

## Previous Milestone: v9.1 Span Fix (2026-03-31)

**Delivered:** AST Span 위치 정보 수정 — PositionedToken 파이프라인으로 에러 메시지에 정확한 file:line:column 표시
- 2 files changed, ~100 LOC
- 659 flt tests, 224 unit tests
- 69 phases, 150 plans executed across v1.0-v9.1

## Previous Milestone: v9.0 Project Build System (2026-03-31)

**Delivered:** funproj.toml 기반 Cargo 스타일 빌드 시스템 — CLI 확장 + 프로젝트 파일 파싱 + build/test 서브커맨드
- 35 files changed, +3,459 LOC in v9.0
- 659 flt tests, 224 unit tests
- 68 phases, 149 plans executed across v1.0-v9.0

## Previous Milestone: v8.1 Mutual Recursion Completion (2026-03-31)

**Delivered:** LetRecDecl/LetRec 첫 파라미터 타입 어노테이션 보존 + expression-level `let rec ... and ... in expr` 상호 재귀
- 27 files changed, +2,007 LOC in v8.1
- 652 flt tests, 224 unit tests
- 66 phases, 142 plans executed across v1.0-v8.1
...
| LetRec/LetRecDecl에 TypeExpr option 추가 | 첫 파라미터 타입 손실 수정, 최소 변경 (튜플 확장) | ✓ Good |
| LetRec bindings list shape = LetRecDecl shape | 동일 튜플로 통일, LetRecContinuation 재사용 | ✓ Good |
| sharedEnvRef 패턴 (expression-level mutual rec) | LetRecDecl 평가 패턴 그대로 적용, 검증된 접근법 | ✓ Good |
| elaborateTypeExpr bare function (open Elaborate) | Bidir.elaborateTypeExpr 아닌 직접 호출, 3개 파일 동일 패턴 | ✓ Good |
| Prelude priority: --prelude > env > toml > auto | 명확한 우선순위 체인, 각 레벨 독립 테스트 가능 | ✓ Good |
| File import cache: own exports only | Diamond dep 안전, caller env 오염 방지 | ✓ Good |
| Tomlyn 2.3.0 + [<CLIMutable>] POCO | TOML 1.1 준수, reflection 기반 자동 매핑 | ✓ Good |
| Argu [<CliPrefix(CliPrefix.None)>] on Build/Test cases | 기존 flat flag DU 구조 유지하면서 서브커맨드 추가 | ✓ Good |
| funproj.toml paths relative to project dir | Cargo 관례 준수, CWD 독립적 | ✓ Good |

---
*Last updated: 2026-03-31 after v10.0 milestone started*
