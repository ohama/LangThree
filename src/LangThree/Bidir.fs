...
let rec synth (ctorEnv: ConstructorEnv) (ctx: InferContext list) (env: TypeEnv) (expr: Expr): Subst * Type =
    ...
    | App (func, arg, span) ->
        ...
        let s2, argTy = synth ctorEnv (InAppArg span :: ctx) (applyEnv s1 env) arg
    ...
    | Lambda (param, body, _) ->
        ...
        let s, bodyTy = synth ctorEnv ctx bodyEnv body
    ...
    | Annot (e, tyExpr, span) ->
        ...
        let s = check ctorEnv ctx' env e expectedTy
        (s, apply s expectedTy)

    // === Let (BIDIR-07 - let-polymorphism) ===
    | Let (name, value, body, span) ->
        let s1, valueTy = synth ctorEnv (InLetRhs (name, span) :: ctx) env value
        let env' = applyEnv s1 env
        let scheme = generalize env' (apply s1 valueTy)
        let bodyEnv = Map.add name scheme env'
        let s2, bodyTy = synth ctorEnv (InLetBody (name, span) :: ctx) bodyEnv body
    ...
    | LetRec (name, param, body, expr, span) ->
        ...
        // Infer body type
        let s1, bodyTy = synth ctorEnv (InLetRecBody (name, span) :: ctx) bodyEnv body
        ...
        let s3, exprTy = synth ctorEnv ctx exprEnv expr
        (compose s3 s, exprTy)

    // === If ===
    | If (cond, thenExpr, elseExpr, span) ->
        let s1, condTy = synth ctorEnv (InIfCond span :: ctx) env cond
        let s2, thenTy = synth ctorEnv (InIfThen span :: ctx) (applyEnv s1 env) thenExpr
        let s3, elseTy = synth ctorEnv (InIfElse span :: ctx) (applyEnv (compose s2 s1) env) elseExpr
    ...
    | Add (e1, e2, span) ->
        let s1, t1 = synth ctorEnv ctx env e1
        let s2, t2 = synth ctorEnv ctx (applyEnv s1 env) e2
    ...
    | Subtract (e1, e2, _) | Multiply (e1, e2, _) | Divide (e1, e2, _) ->
        let s = inferBinaryOp ctorEnv ctx env e1 e2 TInt TInt
        (s, TInt)

    | Negate (e, _) ->
        let s, t = synth ctorEnv ctx env e
    ...
    | And (e1, e2, _) | Or (e1, e2, _) ->
        let s = inferBinaryOp ctorEnv ctx env e1 e2 TBool TBool
        (s, TBool)

    // === Tuple (BIDIR-03) ===
    | Tuple (exprs, span) ->
        let folder (s, tys, idx) e =
            let s', ty = synth ctorEnv (InTupleElement (idx, span) :: ctx) (applyEnv s env) e
    ...
    | List (exprs, span) ->
        ...
        | [] ->
            ...
            (empty, TList elemTy)
        | first :: rest ->
            let s1, elemTy = synth ctorEnv (InListElement (0, span) :: ctx) env first
            let folder (s, ty, idx) e =
                let s', eTy = synth ctorEnv (InListElement (idx, span) :: ctx) (applyEnv s env) e
    ...
    // === Cons ===
    | Cons (head, tail, span) ->
        let s1, headTy = synth ctorEnv (InConsHead span :: ctx) env head
        let s2, tailTy = synth ctorEnv (InConsTail span :: ctx) (applyEnv s1 env) tail
    ...
    | Match (scrutinee, clauses, span) ->
        let s1, scrutTy = synth ctorEnv (InMatch span :: ctx) env scrutinee
        let resultTy = freshVar()
        let folder (s, idx) (pat, expr) =
            ...
            // Synth clause body
            let s'', exprTy = synth ctorEnv (InMatchClause (idx, span) :: ctx) clauseEnv expr
    ...
    | LetPat (pat, value, body, span) ->
        let s1, valueTy = synth ctorEnv ctx env value
        let patEnv, patTy = inferPattern ctorEnv pat
        ...
        let s3, bodyTy = synth ctorEnv ctx bodyEnv body
        (compose s3 s, bodyTy)

/// Check expression against expected type (checking mode)
/// Returns: substitution that makes expression have expected type
and check (ctorEnv: ConstructorEnv) (ctx: InferContext list) (env: TypeEnv) (expr: Expr) (expected: Type): Subst =
    match expr with
    ...
    | If (cond, thenExpr, elseExpr, span) ->
        ...
        let s3 = check ctorEnv (InIfThen span :: ctx) (applyEnv s12 env) thenExpr (apply s12 expected)
        let s4 = check ctorEnv (InIfElse span :: ctx) (applyEnv (compose s3 s12) env) elseExpr (apply (compose s3 s12) expected)
...
and inferBinaryOp ctorEnv ctx env e1 e2 leftTy rightTy =
    let s1, t1 = synth ctorEnv ctx env e1
    let s2, t2 = synth ctorEnv ctx (applyEnv s1 env) e2
...
let synthTop (env: TypeEnv) (expr: Expr): Type =
    let s, ty = synth Map.empty [] env expr
    apply s ty
...
let synthTopWithCtors (ctorEnv: ConstructorEnv) (env: TypeEnv) (expr: Expr): Type =
    let s, ty = synth ctorEnv [] env expr
    apply s ty
