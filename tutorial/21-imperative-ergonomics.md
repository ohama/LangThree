...
v5.0에서 LangThree는 명령형 스타일 코드를 더 자연스럽게 작성할 수 있는 네 가지 문법 기능을 추가했습니다. 이 장에서는 표현식 시퀀싱, 루프, 인덱싱 문법, else 없는 if 표현식을 살펴봅니다.

## 표현식 시퀀싱 (Expression Sequencing)

함수형 언어에서 여러 부수 효과(side effect)를 순서대로 실행하려면 전통적으로 `let _ = e1 in e2` 패턴을 사용했습니다. v5.0에서는 `;` 연산자로 이를 간결하게 쓸 수 있습니다.
...
v5.0에서는 `arr.[i]` 형태의 인덱싱 문법이 추가되어 배열과 해시테이블에 더 직관적으로 접근할 수 있습니다.
