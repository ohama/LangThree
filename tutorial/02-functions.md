...
두 코드가 완전히 동일하다는 점이 커링의 핵심입니다. `add x y = x + y`는 문법적 편의를 위한 축약 표현일 뿐, 실제로는 `fun x -> fun y -> x + y`입니다. 다중 파라미터 람다 `fun x y -> x + y`로도 쓸 수 있으며, 이 역시 중첩된 람다로 디슈거됩니다. `add 3 4`를 평가하면 먼저 `add 3`이 `fun y -> 3 + y`라는 새로운 함수를 반환하고, 거기에 `4`를 적용해 `7`을 얻습니다.
