...
왜 주석이 필수인지 이해하려면, 컴파일러의 입장에서 생각해봐야 합니다. 컴파일러는 `e`의 타입이 `'a Expr`이라는 것을 알지만, `'a`가 무엇인지는 모릅니다. 타입 주석 `: int`를 통해 "이 match의 결과가 `int`가 되어야 한다"고 알려주면, 컴파일러는 **검사 모드(check mode)**에 진입하여 각 분기를 분석할 때 GADT의 생성자 타입 정보를 활용할 수 있습니다.

OCaml에서는 이것을 "locally abstract types"라는 메커니즘으로 처리하고, Haskell에서는 더 깊이 통합되어 있어서 명시적 주석이 불필요한 경우가 많습니다. LangThree는 명시적 주석을 요구하여, 코드를 읽는 사람이 "여기서 GADT 타입 정제가 일어나고 있다"는 것을 바로 알 수 있게 합니다.
...
| match 주석 | 대부분 불필요 (타입 추론) | 필수 (`(match ... : int)`) |
| 다중 타입 반환 | `eval :: Expr a -> a` 가능 | `eval`이 하나의 구체적 타입 반환 |
...
OCaml에서 `type a.`는 **locally abstract type** 선언입니다. 이것이 LangThree의 `(match ... : int)` 주석과 같은 역할을 합니다 — 컴파일러에게 "이 함수 안에서 GADT 타입 정제를 수행하라"고 알려줍니다. 주석 없이는 OCaml도 타입 정제를 할 수 없습니다. LangThree가 주석을 요구하는 것은 OCaml의 이 설계를 따른 것입니다.
...
| **LangThree** | 네이티브 | `IntLit : int -> int Expr` | `(match ... : int)` 필요 | 고정 반환 타입만 |
